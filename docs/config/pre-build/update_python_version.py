...
def on_page_markdown(markdown, **kwargs):
    ...
    """
    if (
        "<!--MIN-PYTHON-VERSION-->" in markdown
        or "<!--MAX-PYTHON-VERSION-->" in markdown
        or "<!--PYTHON-VERSION-RANGE-->" in markdown
    ):
        ...
        if max_version:
            version_range = f"{min_version} to {max_version}"
        else:
            version_range = f"{min_version} or later"

        # Replace placeholders
        markdown = markdown.replace("<!--MIN-PYTHON-VERSION-->", min_version)
        markdown = markdown.replace("<!--MAX-PYTHON-VERSION-->", max_version or "later")
        markdown = markdown.replace("<!--PYTHON-VERSION-RANGE-->", version_range)
