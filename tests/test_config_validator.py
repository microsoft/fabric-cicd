...
class TestDuplicateKeyDetection:
    ...
    def test_duplicate_top_level_key_detected(self, tmp_path):
        """Test that duplicate top-level YAML keys are rejected."""
        config_file = tmp_path / "config.yaml"
        ...
        assert result is None
        assert len(self.validator.errors) == 1
        assert "Duplicate key 'core'" in self.validator.errors[0]

    def test_duplicate_nested_key_detected(self, tmp_path):
        """Test that duplicate nested YAML keys are rejected."""
        config_file = tmp_path / "config.yaml"
        config_file.write_text("core:\n  workspace_id: id1\n  workspace_id: id2\n")
        ...
        assert result is None
        assert len(self.validator.errors) == 1
        assert "Duplicate key 'workspace_id'" in self.validator.errors[0]

    def test_no_duplicate_keys_passes(self, tmp_path):
        ...
        assert result == {"core": {"workspace_id": "id1", "repository_directory": "/path"}}
        assert self.validator.errors == []

    def test_duplicate_key_in_integration(self, tmp_path):
        """Test duplicate key detection through validate_config_file."""
        config_file = tmp_path / "config.yaml"
        config_file.write_text(
            "core:\n"
            "  workspace_id: 12345678-1234-1234-1234-123456789abc\n"
