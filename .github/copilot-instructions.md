...
- `DefaultAzureCredential` fallback is deprecated and will be removed in a future release. You must provide an explicit `token_credential` parameter when initializing `FabricWorkspace`.
- Requires explicit credential methods: `AzureCliCredential`, `AzurePowerShellCredential`, `ManagedIdentityCredential`, or `ClientSecretCredential`
- For local development: Use `AzureCliCredential` (`az login`) or `AzurePowerShellCredential` (`Connect-AzAccount`)
- For CI/CD pipelines: Use `ManagedIdentityCredential` for Azure-hosted runners or `ClientSecretCredential` for service principals
- Fabric Notebook runtime: Authentication handled automatically
