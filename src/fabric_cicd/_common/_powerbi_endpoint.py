...
"""Handles interactions with the PowerBI API, including authentication and request management."""
...
class PowerBiEndpoint:
    """Handles interactions with the PowerBi API, including authentication and request management."""

    def __init__(self, token_credential_powerbi: TokenCredential, requests_module: requests = requests) -> None:
        """
        Initializes the PowerBiEndpoint instance, sets up the authentication token.

        Args:
            token_credential_powerbi: The token credential.
            requests_module: The requests module.
        """
        self.aad_token_powerbi = None
        self.aad_token_expiration_powerbi = None
        self.token_credential_powerbi = token_credential_powerbi
        self.requests_powerbi = requests_module
        self._refresh_token()
        ...
        while not exit_loop:
            try:
                ...
                response = self.requests_powerbi.request(
                    method=method, url=url, headers=headers, json=body, files=files
                )
    ...
    def _refresh_token(self) -> None:
        ...
        if (
            self.aad_token_powerbi is None
            or self.aad_token_expiration_powerbi is None
            or self.aad_token_expiration_powerbi < datetime.datetime.utcnow()
        ):
            ...
            try:
                decoded_token = _decode_jwt(self.aad_token_powerbi)
                ...
                if expiration:
                    self.aad_token_expiration_powerbi = datetime.datetime.fromtimestamp(expiration)
                else:
