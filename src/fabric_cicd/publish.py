...
def unpublish_all_orphan_items(
    ...
    Note:
        By default, the Fabric Delete Item API performs a soft delete, moving items to the workspace recycle bin.
        However, not all item types support soft delete — for unsupported types, the only option is
        to perform a hard (permanent) delete via enabling the ``enable_hard_delete`` feature flag.
        Please note that hard delete requires the workspace **Admin** role.
